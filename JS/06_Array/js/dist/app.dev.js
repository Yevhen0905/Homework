...
show(listOfProducts); // другий варіант
...
var copyListOfProducts = [].concat(listOfProducts);
show(copyListOfProducts);
copyListOfProducts.splice(1, 1);
show(copyListOfProducts); // 2. Додавання покупки в список. Враховуй, що при додаванні покупки
...
function addToListOfProducts(addProductName) {
  var findedItem = listOfProducts.findIndex(function (item) {
    return item.productName === addProductName.productName;
...
show(listOfProducts); // let addProductName = {
//     productName: 'pear',
//     quantity: 1,
//     status: 'no bought',
//     price: 35,
...
var res = listOfProducts.reduce(function (acc, item) {
  return acc += item.quantity * item.price;
}, 0);
show(res); //другий варіант
...
var sum = listOfProducts.reduce(function (acc, item) {
  return acc += item.quantity;
}, 0);
show(sum); // другий варіант
...
showlistOfProducts('down', listOfProducts); // showlistOfProducts('Up', listOfProducts);