...
var LI_CLASS = 'list-group-item d-flex align-items-center border-0 mb-2 rounded'; // з великої літери константа буду часто використовуватися і є статичною
...
function toggleModal() {
  // універсальна функція відкриття і закриття модалки
  var modalW = document.querySelector('.modal'); // знайшли модалку

  var isOpen = modalW.classList.contains('open'); //перевіряємо є класс 'open' чи ні 
...
function createIcon() {
  // створюємо "а" для відкриття модального вікна і добавляємо до li
  var a = document.createElement('a');
  a.setAttribute('href', '#');
  a.classList.add('show-icon');
  a.addEventListener('click', function (e) {
    // для відкривання модалки
    var targetItem = e.target.parentNode; // хочемо знайти li в середені якого іконка по якій клікаємо(шоб добавити підпис модального вікна)
    // e.target дасть нам  "а" по якому ми клікаємо .parentNode його батківський елемент

    var dataIndex = targetItem.dataset.index; // отримати доступ до елементу 'li' де інформація яку я буду відображати
    //беру дата-атрибут батьківського li

    var title = document.querySelector('.modal-title'); // знаходимо html елемент де будемо писати текст
    ...
    var badge = document.querySelector('.badge'); // знаходимо клас статуса і підв'язуємо
    ...
    if (todoArr[dataIndex].isDone) {
      // зміна кольору в залежності від статусу
...
function createLi(item, index) {
  var li = document.createElement('li'); //створюємо li

  var checkbox = createCheckbox(); //запускаємо функцію створений инпут в li

  var icon = createIcon(); // запускаємо функцію створений 'a' в li
...
function createCheckbox() {
  ...
  checkbox.setAttribute('class', 'form-check-input me-2'); // шукаємо необхідний елемент із масиву і змінювати

  checkbox.addEventListener('change', function (e) {
    //буква 'e' сам об'єкт подій, містить дом елемент де ця подія проходить
    var targetCheckbox = e.target;
    var isChecked = targetCheckbox.checked;
    var parentLi = targetCheckbox.parentNode; //parentNode взяти батьківський елемент

    parentLi.classList.toggle('active');
    var dataIndex = parentLi.dataset.index; // dataset для дата - атрибутів
...
function createToDo() {
  // функція створення li , проходмо по масиву
  todoArr.forEach(function (item, index) {
    var li = createLi(item, index); //займе це місце 
...
function addToDo() {
  var inputEl = document.getElementById('createTodoInput'); // знайшли на сторінці импут для ведення value

  var newToDoName = inputEl.value; // прочитали value // тепер добавляємо атрибут onclick (події), при нажимані на кнопку запусти функцію з js. при нажимані знаходить наш инпут і бере значення, яке буде новою назвою тудушки

  var newToDoObj = {
    //так як массив складється з об'єкт, потрубно його сформувати, щоб він створювався
    label: newToDoName,
    isDone: false
